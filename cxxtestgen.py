...
def generateTests(arguments):
    '''The main program'''
    files = parseCommandline(arguments)
...
def parseCommandline(arguments):
    ...
    try:
        options, patterns = getopt.getopt(arguments, 'o:r:',
...
suite_re = re.compile( r'\bclass\s+(\w+)\s*:\s*public\s+((::)?\s*CxxTest\s*::\s*)?(\w+)Suite\b' )
